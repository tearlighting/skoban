...
import { filterMapByType } from "../utils"
import { Ruler } from "../oop/ruler/Ruler"
...
export function usePushBox() {
  ...
  function initGame({ map, player, boxes, targets }: IPushBoxInitParam) {
    ...
    gameData.boxes = boxes.map((b) =>
      useBox({ position: b, targets: targetsReactive })
    ) as any
    const { onTarget } = gameData.boxes[0]
    console.log("raw", onTarget)

    gameData.walls = filterMapByType(map, ECellType.Wall).map((w) =>
      useWall({ position: w })
    )
    gameData.floors = filterMapByType(map, ECellType.Floor).map((f) =>
      useFloor({ position: f })
    )
    rulerIns.init({
  ...
  function move(direction: EDirection) {
    ...
    const { value, box } = rulerIns.canMove(direction)
    if (value) {
...
function getPlayerRenderData(
  playerStore: Reactive<IPushBoxGameData["player"]>
) {
  if (!playerStore) return {}
  const { player: cell, onTarget } = playerStore
  return {
    class:
      getImgClass(cell.type) + getOnTargetClass({ onTarget: onTarget as any }),
    style: getCellStyle(cell),
  }
}

function getBoxRenderData(
  boxStore: Reactive<IPushBoxGameData["boxes"][number]>
) {
...
function getTargetRenderData(
  targetStore: Reactive<IPushBoxGameData["targets"][number]>
) {
...
function getWallRenderData(
  wallStore: Reactive<IPushBoxGameData["walls"][number]>
) {
...
function getFloorRenderData(
  floorStore: Reactive<IPushBoxGameData["floors"][number]>
) {
